 ...
 * @author Jeff
 */
class Yparameter extends Admin_Controller 
{
	/**
	 * Constructor
	 */
	...
	function index()
	{
		$this->template->build('yparameter/index');
	}
	function load(){
		$edit	= site_url('inventory/yparameter/form/$1');		
		$treeview	= site_url('organization/nodegroup/treeview/$1');		
		$link	= '	
		...
		$this->datatables->where('a.type_id', '2');
	...
	{	
		...
		$this->data['parent']	= $this->mgeneral->GetWhere(array('type_id'=>2),"tr_item_spc");
		if(!isset($id)){
			
			$this->template->build('yparameter/add');
		}else{
			
			$this->data['edit']	= $this->mgeneral->GetRow(array('id'=>$id),"tr_item_spc");
			$this->template->build('yparameter/edit');
		}
	}
	/**
