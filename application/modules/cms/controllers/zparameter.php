 ...
 * @author Jeff
 */
class Zparameter extends Admin_Controller 
{
	/**
	 * Constructor
	 */
	...
	function index()
	{
		$this->template->build('zparameter/index');
	}
	function load(){
		$edit	= site_url('inventory/zparameter/form/$1');		
		...
		$this->datatables->where('a.type_id', '3');
	...
	{		
		...
		$this->data['parent']	= $this->mgeneral->GetWhere(array('type_id'=>3),"tr_item_spc");
		if(!isset($id)){
			
			$this->template->build('zparameter/add');
		}else{
			
			$this->data['edit']	= $this->mgeneral->GetRow(array('id'=>$id),"tr_item_spc");
			$this->template->build('zparameter/edit');
		}
	}
	/**
