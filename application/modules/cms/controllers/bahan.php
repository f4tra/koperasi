...
class Bahan extends Admin_Controller 
{
	...
	function index()
	{
		$this->template->build('bahan/index');
	}
	function load(){
		$edit	= site_url('inventory/bahan/form/$1');				
		...
		$this->datatables->where('a.type_id', '5');
	...
	{		
		...
		$this->data['parent']	= $this->mgeneral->GetWhere(array('type_id'=>5),"tr_item_spc");
		if(!isset($id)){
			$this->template->build('bahan/add');
		}else{
			$this->data['edit']	= $this->mgeneral->GetRow(array('id'=>$id),"tr_item_spc");
			$this->template->build('bahan/edit');
		}
	}
	/**
