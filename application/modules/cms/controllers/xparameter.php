 ...
 * @author Jeff
 */
class Xparameter extends Admin_Controller 
{
	/**
	 * Constructor
	 */
	...
	function index()
	{
		$this->template->build('xparameter/index');
	}
	function load(){
		$edit	= site_url('inventory/xparameter/form/$1');				
		...
		$this->datatables->where('a.type_id', '1');
	...
	{		
		...
		$this->data['parent']	= $this->mgeneral->GetWhere(array('type_id'=>1),"tr_item_spc");
		if(!isset($id)){
			$this->template->build('xparameter/add');
		}else{
			$this->data['edit']	= $this->mgeneral->GetRow(array('id'=>$id),"tr_item_spc");
			$this->template->build('xparameter/edit');
		}
	}
	/**
