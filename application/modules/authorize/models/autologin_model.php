...
class Autologin_model extends CI_Model {
    ...
    public function update($user, $series, $private) {
        ...
        return $this->db->update($this->table, array('key' => $private, 'created' => date('Y-m-d H:i:s')));
    }
    ...
    public function insert($user, $series, $private) {
        return $this->db->insert($this->table, array('user' => $user, 'series' => $series, 'key' => $private, 'created' => date('Y-m-d H:i:s')));
    }
