...
class Template
{
		...
		foreach ($options as $key => $value)
		{
			...
			switch ($key)
			{
				...
				case 'css':
					...
					$b = $value['beckend'];
					$c = $value['frontend'];					
					foreach($b as $keys =>$cssval)
					{
						if (is_numeric($keys))
							$this->set_css($cssval);
						elseif (!is_array($cssval))
							$this->set_css($keys, array('media' => $cssval));
						else
							$this->set_css($keys, $cssval);
						
					}
					foreach($c as $csskey => $cssval)
					{
						if (is_numeric($csskey))
							$this->set_css_f($cssval);
						elseif (!is_array($cssval))
							$this->set_css_f($csskey, array('media' => $cssval));
						else
							$this->set_css_f($csskey, $cssval);
					}
					break;
				case 'js_header':
					$b = $value['beckend'];
					...
					foreach($c as $jskey => $jsval)
					{
						if (is_numeric($jskey))
							...
							$this->set_js_script_f($jsval, $jskey);
					}
					break;
				case 'js_footer':
					$b = $value['beckend'];
					...
					{
						if (is_numeric($jskey))
							$this->set_js_f($jsval, TRUE);
						else
							$this->set_js_script_f($jsval, $jskey, TRUE);
					}
					break;
				case 'default_layout':
	...
	public function set_css_f($name, $attr = array('media' => 'screen'))
	{
		$this->css_f[$name] = $attr;
	...
	public function set_js_f($name, $footer = FALSE)
	{
		if ($footer)
			$this->js_footer_f[$name] = $name;
		else
			$this->js_header_f[$name] = $name;
	...
	public function set_js_script_f($script, $name = '', $footer = FALSE)
	{
		if ($footer)
		{
			if (empty($name))
				$name = 'script' . count($this->js_footer_f);
			$this->js_footer_f[$name] = $script;
		}
		else
		{
			if (empty($name))
				$name = 'script' . count($this->js_header_f);
			$this->js_header_f[$name] = $script;
		}
		return $this;
	}
	...
	public function build($view, $data = array(), $output = FALSE)
	{
		...
		foreach($this->css as $key => $value)
		{
			...
			if (!empty($style)){
				...
				$template_data['css']['beckend'] .= "\t\t<style>\r\n" . $style . "\r\n\t\t</style>\r\n";
			...
			else
			{
				if (preg_match('!^\w+://! i', $key)){
					...
					$template_data['css']['beckend'] .= "\t\t<link href=\"{$key}.css\" {$attr}/>\r\n";					
				}	
				else
				{					
		...
		foreach($this->css_f as $key => $value)
		{
			$style = '';
			if (isset($value['style']))
			{
			...
			if (!empty($style)){
				...
				$template_data['css']['frontend'] .= "\t\t<style>\r\n" . $style . "\r\n\t\t</style>\r\n";
			...
			else
			{
				if (preg_match('!^\w+://! i', $key)){
					...
					$template_data['css']['frontend'] .= "\t\t<link href=\"{$key}.css\" {$attr}/>\r\n";					
				}	
				else
				{					
