		...
		<div class="col-md-12">
			...
			<div class="box border green">
				...
				<tbody>
					...
					<a class="btn btn-warning" href="<?php echo site_url('inventory/productcategory/form');?>">Add New</a>
				</div>
...
$(document).ready(function() {
	$('#data').dataTable({
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "<?php echo site_url('inventory/productcategory/load');?>",
		"sServerMethod": "POST",
		...
		"aoColumns" : [		
			...
			{"mData": "g",
				"mRender" : function ( data, type, gpar ) {				
					//console.log(par.g);
					if(gpar.g == null || gpar.g == '0' || gpar.g == 0){
						return "<font color = '#ff0000'>Unidentify</font>"; 			
					}else{
						return gpar.g; 
					} 				
			...
			{"mData": "type",
				"mRender" : function ( data, type, types ) {				
					
					if(types.type == null || types.type == '0' || types.type == 0){
						return "<font color = '#ff0000'>Unidentify</font>"; 			
					}else{
						return types.type; 
					} 				
	...
	$('#data').delegate('.checkbox','change',function() {
		...
		if(this.checked){
			//alert("checked");
			$.ajax({
				url: '<?php echo site_url('inventory/productcategory/execute/active/'); ?>',
				type: 'post',
				data: 'active=1&id='+i,
...
function del(btn)
{
	...
	else
	{
		...
		$.ajax({
			url: '<?php echo base_url().'index.php/inventory/productcategory/execute/delete/';?>'+id,
			type: "POST",
			data:{data_id:id},
			crossDomain:true,
			beforeSend: function(){
