...
<?php
	$strSQL = $this->db->query("select * from tr_hr_fnc where parent_id = 0 order by code")->result();
	foreach ($strSQL as $key => $one_value) {
		...
		<li>
			...
			<ul>			
				<?php
					$sql_parent = $this->db->query("select * from tr_hr_fnc where parent_id = '".$one_value->id."' order by code")->result();
					foreach($sql_parent as $key=>$two_value){
						...
						<li id="<?=$two_value->name; ?>">
							...
							<ul>
								<?php
									$sql_parent_first = $this->db->query("select * from tr_hr_fnc where parent_id = '".$two_value->id."' order by code")->result();
									...
									<li id="<?=$tree_value->name;?>">
										...
										<ul>
											<?php
												$sql_parent_four = $this->db->query("select * from tr_hr_fnc where parent_id = '".$tree_value->id."' order by code")->result();
												...
												?>
													<li id="<?=$tree_value->name;?>">
														...
														<ul>
															<?php
																$sql_parent_five = $this->db->query("select * from tr_hr_fnc where parent_id = '".$four_value->id."' order by code")->result();
