				...
				<tbody>
					...
					<tr>
						...
						<td>
							<?php
							if($user->id ==1){
								$edit	= site_url('hrd/salesarea/form/'.$value->id);		
					...
					<?php
					if($user->id ==1){
						print '<a class="btn btn-warning" href="'.site_url('hrd/salesarea/form').'">Add New</a>';
					}
					?>
...
$(document).ready(function() {
	...
	$('#data').delegate('.checkbox','change',function() {
		...
		if(this.checked){
			//alert("checked");
			$.ajax({
				url: "<?php echo site_url('hrd/salesarea/execute/active/'); ?>",
				type: 'post',
				data: 'active=1&id='+i,
...
function del(btn)
{
	...
	else
	{
		...
		$.ajax({
			url: "<?php echo site_url('hrd/salesarea/execute/delete/');?>/"+id,
			type: "POST",
			data:{data_id:id},
			crossDomain:true,
			beforeSend: function(){
