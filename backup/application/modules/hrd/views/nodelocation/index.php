			...
			<div class="box border green">
				...
				<tbody>
					<tr>
					 ...
					 <a class="btn btn-warning" href="<?php echo site_url('organization/nodelocation/form');?>">Add New</a>
				</div>
...
$(document).ready(function() {
	$('#data').dataTable({
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "<?php echo site_url('organization/nodelocation/load');?>",
		"sServerMethod": "POST",
		...
		"aoColumns" : [
			...
			{"mData": "ctg",
				"mRender" : function ( data, type, par ) {				
					if(par.ctg == 0 ||par.ctg == ''||par.ctg == null){
						return "<font color='#ff0000'>Unidentify</font>"; 			
					}else{
						return par.ctg; 
					} 				
	...
	$('#data').delegate('.checkbox','change',function() {
		...
		if(this.checked){
			//alert("checked");
			$.ajax({
				url: '<?php echo site_url('organization/nodelocation/execute/active/'); ?>',
				type: 'post',
				data: 'active=1&id='+i,
...
function del(btn)
{
	...
	else
	{
		...
		$.ajax({
			url: '<?php echo base_url().'index.php/organization/nodelocation/execute/delete/';?>'+id,
			type: "POST",
			data:{data_id:id},
			crossDomain:true,
			beforeSend: function(){
