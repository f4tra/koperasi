 ...
 * @author Jeff
 */
class Nodegroup extends Admin_Controller 
{
	/**
	 * Constructor
	 */
	...
	function index()
	{
		$this->template->build('nodegroup/index');
	}
	function load(){
		$edit	= site_url('organization/nodegroup/form/$1');		
		...
		$this->datatables->from('tr_node_grp a');
		$this->datatables->join('tr_node_grp b','b.id = a.parent_id','left');
		//$this->datatables->join('tr_company c','c.id = a.issuer_id','left');
	...
	{		
		...
		$this->data['parent']	= $this->mgeneral->GetWhere(array('active'=>1),"tr_node_grp");
		if(!isset($id)){
			$this->template->build('nodegroup/add');
		}else{
			$this->data['edit']	= $this->mgeneral->GetRow(array('id'=>$id),"tr_node_grp");
			$this->template->build('nodegroup/edit');
		}
	}
	/**
	...
	{		
		...
		if($method == "update")
		{
			...
			$this->mgeneral->Update(array('id'=>$id),$data,"tr_node_grp");			
			$result['code'] 	= "01";
		...
		{			
			...
			$this->mgeneral->save($data,"tr_node_grp");
			$result['code'] 	= "02";
		...
		else if ($method=="delete")
		{
			...
			$this->mgeneral->delete(array('id'=>$var),"tr_node_grp");
			$result['code'] 	= "03";
		...
		else if ($method == "active")
		{
			...
			$this->mgeneral->update(array('id'=>$i),$data,"tr_node_grp");
			$result['code'] 	= "04";
