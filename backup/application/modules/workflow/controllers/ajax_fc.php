 ...
 * @author Jeff
 */
class Ajax_fc extends Admin_Controller 
{
	/**
	 * Constructor
	 */
	...
	/*function push_point(){
		...
		$this->mgeneral->save($data,'tr_fc_cnt');
	...
	function push(){
		...
		$code = $this->input->post('code');
		$type = $this->input->post('type');
		$loop 	= $this->mgeneral->getRow(array('code'=>$code),'tr_fc_cnt');
		if(empty($loop)){
			$data = array(
				'from_id'=>$source,
				'to_id'=>$target,
			...
			$this->mgeneral->update(array('id'=>$target),$data_update,"tr_fc");
		}else{
	...
	function push_dragble(){
		...
		$this->mgeneral->Update(array('id'=>$id),$data,"tr_fc");
	}
	function delete(){
		$result = array();
		$var = $this->input->post('data_id');
		$this->mgeneral->delete(array('id'=>$var),"tr_fc");
		$result['code'] 	= "03";
	...
	function push_con_update(){
		...
		$this->mgeneral->update(array('code'=>$var),$data,"tr_fc_cnt");
	}
	function push_con_del(){
		$var = $this->input->post('code');
		$this->mgeneral->delete(array('code'=>$var),"tr_fc_cnt");
	}
	/*
	...
	*/
	function load_pt(){
		...
		$this->datatables->from('tr_fc_type a');
		$this->datatables->join('tr_fc_type b','b.id = a.parent_id','left');
	...
	{		
		...
		{			
			...
			$r = $this->mgeneral->save($data,"tr_fc");
			$this->mgeneral->Update(array('id'=>$r),array('group_id'=>$r),"tr_fc");	
			$result['code'] 	= "02";
		...
		else if ($method == "active")
		{
			...
			$this->mgeneral->update(array('id'=>$i),$data,"tr_fc");
			$result['code'] 	= "04";
	...
	{		
		...
		if($method == "update")
		{
			...
			$this->mgeneral->Update(array('id'=>$id),$data,"tr_fc_type");			
			$result['code'] 	= "01";
			$result['message']	= "Update Sukses";
		}
		else if ($method == "save")
		{			
			...
			$this->mgeneral->save($data,"tr_fc_type");
			$result['code'] 	= "02";
			$result['message']	= "Save Sukses";
		}
		else if ($method=="delete")
		{
			$var = $this->input->post('data_id');
			$this->mgeneral->delete(array('id'=>$var),"tr_fc_type");
			$result['code'] 	= "03";
			$result['message']	= "Delete Sukses";
		}
		else if ($method == "active")
		{
			...
			$this->mgeneral->update(array('id'=>$i),$data,"tr_fc_type");
			$result['code'] 	= "04";
			$result['message']	= "Active";
		}
		else
