				...
				<div class="box-body" id="dropzone" >
		            ...
		            ?>
		                ...
		                <img  width="30px"  src="<?php echo base_url('assets/wf/'.$value->icons.''); ?>" />
		               	<br/>
		               		<a href="#<?php //echo site_url('workflow/group/detail/'.$value->id); ?>
...
(function (interact){
	...
	function setupDropzone(el, accept) {
		...
		.on('drop', function (event) {
			...
			$.ajax({
				url: '<?php echo site_url();?>workflow/ajax_fc/execute/save_detail',
				type: "POST",
...
function delete_node(btn){
	var id = $(btn).data('id');
	$.ajax({
		url: "<?php echo site_url('workflow/ajax_fc/delete'); ?>",
		type:"POST",
		data: {data_id:id},
		success: function(data ) {				    
			window.location.reload();
...
function execute(type){
	switch(type){
		...
		*/
		case 'edit':
			$.ajax({
	  			url: "<?php echo site_url('workflow/ajax_fc/push_con_update'); ?>",
	  			type:"POST",
		...
		case 'delete':
			...
			$.ajax({
	  			url: "<?php echo site_url('workflow/ajax_fc/push_con_del'); ?>",
	  			type:"POST",
	  			data: {code:code},
				success: function(data ) {				    
					window.location.reload();
		...
		case 'edit_node':
			$.ajax({
	  			url: "<?php echo site_url('workflow/ajax_fc/edit'); ?>",
			  	type:"POST",
	...
	var instance = jsPlumb.getInstance({
		...
		instance.draggable(instance.getSelector(".statemachine-demo .w"), { 
			...
			stop:function(event, ui){ 				
				...
				$.ajax({
	  				url: "<?php echo site_url('workflow/ajax_fc/push_dragble'); ?>",
	  				type:"POST",
	  				data: {
		...
		instance.bind("connection", function(info, originalEvent) {
			...
			/*$.ajax({
		  		url: "<?php echo site_url('workflow/ajax_fc/push_point'); ?>",
		  		type:"POST",
		  		data: {
		...
		foreach ($cnt as $key => $value) {
			...
			$s = $this->db->query('select code,name from tr_fc where id="'.$value->from_id.'"')->row();
			$t = $this->db->query('select code,name from tr_fc where id="'.$value->to_id.'"')->row();
		?>
			instance.connect({
		...
		$('#saveButton').click(function(){
			...
			$.each(instance.getConnections(),function (idx, connection) {
				...
				str_two = str_two.replace("c", "");
   				$.ajax({
		  			url: "<?php echo site_url('workflow/ajax_fc/push'); ?>",
		  			type:"POST",
		  			data: {
		    			source: str_one,
		    			target: str_two,
