		...
		<div class="col-md-12">
			...
			<div class="box border green">
				...
				<tbody>
					...
					<a class="btn btn-warning" href="<?php echo site_url('setup/workgroup/form');?>">Add New</a>
				</div>
...
$(document).ready(function() {
	$('#data').dataTable({
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "<?php echo site_url('setup/workgroup/load');?>",
		"sServerMethod": "POST",
		...
		"aoColumns" : [
			...
			{"mData": "parent",
			"mRender" : function ( data, type, fulls ) {				
					if(fulls.parent == null || fulls.parent == '' || fulls.parent ==0){
	...
	$('#data').delegate('.checkbox','change',function() {
		...
		if(this.checked){
			//alert("checked");
			$.ajax({
				url: '<?php echo site_url('setup/workgroup/execute/active/'); ?>',
				type: 'post',
				data: 'active=1&id='+i,
...
function del(btn)
{
	...
	else
	{
		...
		$.ajax({
			url: '<?php echo base_url().'index.php/setup/workgroup/execute/delete/';?>'+id,
			type: "POST",
			data:{data_id:id},
			crossDomain:true,
			beforeSend: function(){
