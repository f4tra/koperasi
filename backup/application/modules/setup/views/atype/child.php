		...
		<div class="col-md-12">
			...
			<div class="box border green">
				...
				<tbody>
					...
					<a class="btn btn-warning" href="<?php echo site_url('setup/atype/form_child/'.$data_id);?>">Add New</a>
				</div>
...
$(document).ready(function() {
	$('#data').dataTable({
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "<?php echo site_url('setup/atype/load_child/'.$data_id.'/'.$data_ids);?>",
		"sServerMethod": "POST",
		...
		"aoColumns" : [		
			...
			{"mData": "type_id",
				"mRender" : function ( data, type, tp ) {				
					//console.log(par.parent_id);
					if(tp.type_id == null || tp.type_id == '0' || tp.type_id == 0){
						return "<font color = '#ff0000'>Unidentify</font>"; 			
					}else{
						return tp.type_id; 
					} 				
	...
	$('#data').delegate('.checkbox','change',function() {
		...
		if(this.checked){
			//alert("checked");
			$.ajax({
				url: '<?php echo site_url('setup/atype/execute_child/active/'); ?>',
				type: 'post',
				data: 'active=1&id='+i,
...
function del(btn)
{
	...
	else
	{
		...
		$.ajax({
			url: '<?php echo base_url().'index.php/setup/atype/execute_child/delete/';?>'+id,
			type: "POST",
			data:{data_id:id},
			crossDomain:true,
			beforeSend: function(){
