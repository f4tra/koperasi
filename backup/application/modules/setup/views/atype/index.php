		...
		<div class="col-md-12">
			...
			<div class="box border green">
				...
				<tbody>
					...
					<a class="btn btn-warning" href="<?php echo site_url('setup/atype/form');?>">Add New</a>
				</div>
...
$(document).ready(function() {
	$('#data').dataTable({
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "<?php echo site_url('setup/atype/load');?>",
		"sServerMethod": "POST",
		...
		"aoColumns" : [		
			...
			{"mData": "id",
				"mRender" : function ( data, type, idx ) {	
					//alert(idx.id);			
					return "<a title='View "+idx.name+"' class='btn btn-purple btn-m' href='<?php echo site_url('setup/atype/child/');?>/"+idx.id+"'><i class='fa fa-tag'></i></a> <a title='Edit "+idx.name+"' class='btn btn-warning btn-m' href='<?php echo site_url('setup/atype/form/');?>/"+idx.id+"'><i class='fa fa-pencil'></i></a> <button title='Delete "+idx.name+"' class='btn btn-danger btn-m' data-id='"+idx.id+"' onclick='del(this);''><i class='fa fa-trash-o'></i></button>"; 			
	...
	$('#data').delegate('.checkbox','change',function() {
		...
		if(this.checked){
			//alert("checked");
			$.ajax({
				url: '<?php echo site_url('setup/atype/execute/active/'); ?>',
				type: 'post',
				data: 'active=1&id='+i,
...
function del(btn)
{
	...
	else
	{
		...
		$.ajax({
			url: '<?php echo base_url().'index.php/setup/atype/execute/delete/';?>'+id,
			type: "POST",
			data:{data_id:id},
			crossDomain:true,
			beforeSend: function(){
