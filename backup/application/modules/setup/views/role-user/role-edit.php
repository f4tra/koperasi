				...
				<div class="box-body">
					...
					<form class="form-horizontal row-border"  method="post" id="form">
						...
						<div class="form-group">
							...
							<div class="col-md-4">
								<select class="form-control" name="isrepp" id="isrepp" class="select2-01 col-md-12">
									<option <?php if($edit->isrepp == 1){echo "selected";}else{echo "";}?> value="1">Internal</option>
									<option <?php if($edit->isrepp == 2){echo "selected";}else{echo "";}?> value="2">External</option>
								</select>
							</div>
						</div>
						<div class="form-group">
...
function edit(){
	$('#form').validate({
		...
		submitHandler: function(form){
			$.ajax({
				url: '<?php echo base_url();?>index.php/setup/role/execute/update/<?php echo $edit->id;?>',
				type: "POST",
				dataType:"json",
				data: $("#form").serialize(),
				beforeSend: function(){
