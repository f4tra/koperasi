...
class Nodelocation extends Admin_Controller 
{
	...
	function load(){
		$edit	= site_url('organization/nodelocation/form/$1');		
		...
		$this->datatables->join('tr_node_group c','c.id = b.grp_id','left');
		$this->datatables->join('tr_node_category d','d.id = b.ctg_id','left');
		$this->datatables->join('tr_node_type e','e.id = b.type_id','left');
