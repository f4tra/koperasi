				...
				<div class="box-body">
					...
					<div class="demo statemachine-demo" id="statemachine-demo">
		                ...
		                foreach ($data as $key => $value) {
		                	...
		                	echo '<div  class="w" id="c'.$value->id.'" ><a href="'.site_url('tm/atype/form_child/').'/'.$value->type_id.'/'.$value->id.'">'.$value->code." - ".$value->name.'</a><br/><br/>';
		                	$count_par = $this->db->query('select count(*) as jums from tr_wf_reff where parent_id="'.$value->id.'"')->row();
		                	//print_r($count_par);
		                	if($count_par->jums > 0){
		                		echo '<a title="Show Data List '.$type->name." ".$value->name.' " href="'.site_url('tm/atype/child').'/'.$value->type_id.'/'.$value->id.'"" ><i class="fa fa-list-ol"></i> </a>';
		                		echo '<a title="Show Work Flow '.$type->name." ".$value->name.' " href="'.site_url('tm/atype/widget_a').'/'.$value->type_id.'/'.$value->id.'""  ><i class="fa fa-sitemap"></i> </a>';
		                	}
	...
	var instance = jsPlumb.getInstance({
		...
		instance.draggable(instance.getSelector(".statemachine-demo .w"), { 
			...
			stop:function(event, ui){ 				
				...
				$.ajax({
	  				url: "<?php echo site_url('tm/atype/push_dragble'); ?>",
	  				type:"POST",
	  				data: {
		...
		instance.bind("click", function(c) { 
			...
			if(r == true){
				...
				$.ajax({
	  				url: "<?php echo site_url('tm/atype/push_con_del'); ?>",
	  				type:"POST",
	  				data: {
	    				code : gj	    				
	  				},
			...
			instance.connect({
				...
				overlays : [
					["Label", {													   					
						...
						label : "<br><br>"+"<a href='<?php echo site_url('tm/atype/connector').'/c'.$value->from_id.'c'.$value->to_id; ?>' > <?php echo $s->code." ".$t->code;?></a>", 
						//location:0.7,
						//id:"label",
						/*events:{
		...
		$('#saveButton').click(function(){
			...
			$.each(instance.getConnections(),function (idx, connection) {
				...
				str_two = str_two.replace("c", "");
   				...
   				$.ajax({
	  			url: "<?php echo site_url('tm/atype/push'); ?>",
	  			type:"POST",
	  			data: {
	    			source: str_one,
	    			target: str_two,
