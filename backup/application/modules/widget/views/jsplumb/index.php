	...
	<div class="row">
		<div class="col-md-12">
			...
			<div class="box border green">
				<div class="box-title">
					<h4><i class="fa fa-table"></i>Test Jsplumb</h4> 					
				</div>
				<div class="box-body">
					<!-- demo -->		           
					...
					<div class="demo statemachine-demo" id="statemachine-demo">
		                <?php foreach ($variable as $key => $value) {
		                	# code...
		                	echo '<div  class="w" id="c'.$value->id.'" >'.$value->text.'<div class="ep"></div></div>';
		                }
		                ?>		                                      
		            </div>
                <div id="list"></div>
...
<style type="text/css">
	<?php
		foreach ($variable as $key => $value) {
			echo "#c".$value->id."{left:".$value->left."em;top:".$value->top."em;width:".$value->width."em;}";
		}
	?>
</style>
...
jsPlumb.ready(function() {
	...
	instance.draggable(windows);
		...
		?>
			instance.connect({ source:"c<?php echo $value->source; ?>", target:"c<?php echo $value->target; ?>"});
		...
		$('#saveButton').click(function(){
			...
			$.each(instance.getConnections(),function (idx, connection) {
				...
				str_two = str_two.replace("c", "");
   				...
   				$.ajax({
	  			url: "<?php echo site_url('widget/jsplumb/push'); ?>",
	  			type:"POST",
	  			data: {
	    			source: str_one,
	    			target: str_two,
