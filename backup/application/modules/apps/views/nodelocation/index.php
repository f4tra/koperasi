			...
			<div class="box border green">
				...
				<tbody>
					<tr>
					 ...
					 <a class="btn btn-warning" href="<?php echo site_url('apps/nodelocation/form');?>">Add New</a>
				</div>
...
$(document).ready(function() {
	$('#data').dataTable({
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "<?php echo site_url('apps/nodelocation/load');?>",
		"sServerMethod": "POST",
	...
	$('#data').delegate('.checkbox','change',function() {
		...
		if(this.checked){
			//alert("checked");
			$.ajax({
				url: '<?php echo site_url('apps/nodelocation/execute/active/'); ?>',
				type: 'post',
				data: 'active=1&id='+i,
...
function del(btn)
{
	...
	else
	{
		...
		$.ajax({
			url: '<?php echo base_url().'index.php/apps/nodelocation/execute/delete/';?>'+id,
			type: "POST",
			data:{data_id:id},
			crossDomain:true,
			beforeSend: function(){
