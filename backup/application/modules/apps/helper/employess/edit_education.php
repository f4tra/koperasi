				...
				<div class="box-body">
					...
					<form class="form-horizontal row-border"  method="post" id="form">
								...
								<select  name="education_type" id="education_type" class="select2-01 col-md-12 full-width-fix">
									...
									<?php 
										foreach($TR_HR_EDUCATION_TYPE as $key => $value) {
											if($edit->TYPE_ID == $value->id){
												$selected = "selected";
											}else{
												$selected = "";
											}
											print '<option '.$selected.' value="'.$value->id.'">'.$value->code.' - '.$value->name.'</option>';
										}
									?>									
								</select>
							</div>
						...
						<div class="form-group">
							...
							<div class="col-md-4">
								<textarea name="descr" id="descr" cols="30" rows="10" class="form-control"><?php echo $edit->DESCRIPTION;?></textarea>
							</div>
						</div>						
						...
						<div class="form-group">
							...
							<div class="col-md-4">
								<input value="<?php echo $edit->ACHIEVEMENT;?>"  type="text" name="achievement" id="achievement" class="form-control" placeholder="ACHIEVEMENT" />
							</div>
						</div>
						<div class="form-group">
...
function save(){
	$('#form').validate({
		...
		submitHandler: function(form){
			$.ajax({
				url: '<?php echo base_url();?>index.php/organization/employess/execute_hr_edu/update/<?php echo $edit->id;?>',
				type: "POST",
				dataType:"json",
				data: $("#form").serialize(),
				beforeSend: function(){
