		...
		<div class="col-md-12">
			...
			<div class="box border green">
				...
				<tbody>
					...
					<a class="btn btn-warning" href="<?php echo site_url('apps/employess/form');?>">Add New</a>
				</div>
...
$(document).ready(function() {
	$('#data').dataTable({
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "<?php echo site_url('apps/employess/load');?>",
		"sServerMethod": "POST",
	...
	$('#data').delegate('.checkbox','change',function() {
		...
		if(this.checked){
			//alert("checked");
			$.ajax({
				url: "<?php echo site_url('apps/employess/execute/active/'); ?>",
				type: 'post',
				data: 'active=1&id='+i,
			...
			//alert("unchecked");
			$.ajax({
				url: "<?php echo site_url('organization/employess/execute/active/'); ?>",
				type: 'post',
				data: 'active=0&id='+i,							
				success: function(result)
				{
...
function del(btn)
{
	...
	else
	{
		...
		$.ajax({
			url: '<?php echo site_url().'apps/employess/execute/delete/';?>'+id,
			type: "POST",
			data:{data_id:id},
			crossDomain:true,
			beforeSend: function(){
...
function print(action){
	...
	$.ajax({
		url: '<?php echo site_url().'apps/employess/convert_to_pdf/';?>'+id,
		type: "POST",
		data:{data:id},
		crossDomain:true,
		beforeSend: function(){
