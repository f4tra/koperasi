...
$(document).ready(function() {
  ...
  $('#button-reset').click(function() { plot1.resetZoom() });
	$('#data').dataTable({
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "<?php echo site_url('unitstock/pricehistory/load');?>/<?php if(!empty($idx)) echo $idx;?>",
		"sServerMethod": "POST",
		...
		"aoColumns" : [
			...
			{"mData": "xname",
				"mRender" : function ( data, type, x ) {				
					if(x.xname == 0 ||x.xname == ''||x.xname == null){
						return "<font color='#ff0000'>Unidentify</font>"; 			
					}else{
						return x.xname; 
					} 				
			...
			{"mData": "xname",
				"mRender" : function ( data, type, z ) {				
					if(z.zname == 0 ||z.zname == ''||z.zname == null){
						return "<font color='#ff0000'>Unidentify</font>"; 			
					}else{
						return z.zname; 
					} 				
...
function del(btn)
{
	...
	else
	{
		...
		$.ajax({
			url: '<?php echo base_url().'index.php/unitstock/pricehistory/execute/delete/';?>'+id,
			type: "POST",
			data:{data_id:id},
			crossDomain:true,
			beforeSend: function(){
