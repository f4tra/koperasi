...
class Pengajuan extends Admin_Controller 
{
	...
	function index()
	{
		...
		elseif($this->data['user']->role_id==10){
			$query = $this->db->query("
				select
					a.id as id,
					a.code as code,				
				...
				from tt_purchasing a	
					left join tr_role p on p.id=a.role_id						
				where a.type_id =1 or a.status_id = 11 or a.status_id = 4 or status_id = 0
				")->result();	
		}
		elseif($this->data['user']->role_id ==4){
			$query = $this->db->query("
				select
					a.id as id,
					a.code as code,				
				...
				from tt_purchasing a	
					left join tr_role p on p.id=a.role_id						
				where a.type_id =1 and a.status_id = 1 or a.status_id = 4 or status_id = 0 and role_id = '4'
				")->result();	
		}else{
			$query = $this->db->query("
				select
		...
		$this->template->build('pengajuan_view/index');		
	}
	function form($idx = ''){
		$uid 	= $this->auth->userid();
		...
		if(empty($idx)){
			...
			$this->data['code'] = $code;
			$this->template->build('pengajuan_view/add');				
		}else{
			$this->template->build('pengajuan_view/form_detail');				
	...
	public function detail($idx='')
	{
		...
		$this->data['purchasing_detail'] = $this->mgeneral->getWhere(array('purchasing_id'=>$idx),'tt_purchasing_detail');
		$this->data['purchasing_detail_jum'] = $this->db->query("select sum(price*qty) as total,qty from tt_purchasing_detail where purchasing_id='".$idx."'")->row();
		$this->template->build('pengajuan_view/detail');				
	}
	/**
